...
deleteFavPrd(e:Event,prd:string){
  e.stopPropagation()
  this.favoriteService.delFavProduct(this.id, prd).pipe(
    catchError((error) => {
      return of(null);
    })
  ).subscribe((res)=>{
    this.getFavProducts()
    this.toast.warning({detail:"Favourites",summary:'Product removed from favourites',duration:5000, position:'topRight'});
  })
}
}
